...
def generate_keyword():
    ...
    style = data.get("style", "tang")  # tang/song
    ...
    try:
        ...
        raw_poem = gen.generate(keyword, num_lines=num_lines,
                                temperature=temperature, style=style)

        # BART 输出已分行 + 韵脚修正，直接用换行符拆分
        if "\n" in raw_poem:
            poem_lines = [l.strip() for l in raw_poem.split("\n") if l.strip()]
        else:
...
def _build_scene_prompt(keywords: list, sentiment: dict, scene_text: str) -> str:
    ...
    return f"{kw_str}，意境{mood}。{scene_text}"


def _build_image_prompt(labels: list, description: str) -> str:
    """直接用翻译后的中文描述作为 prompt（BART 理解完整句子比关键词更好）"""
    # 截断过长描述
    desc = description[:100] if description else "自然风景"
    return desc
